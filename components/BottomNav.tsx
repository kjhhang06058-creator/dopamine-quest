...
export function BottomNav({ active, onChange }: { active: Tab; onChange: (t: Tab) => void }) {
  return (
    <nav className="grid grid-cols-3 border-t border-[var(--theme-border)] bg-zinc-950/95 backdrop-blur">
      {TABS.map(({ id, label, icon: Icon }) => (
        <button
          key={id}
          onClick={() => onChange(id)}
          className={`flex flex-col items-center gap-1 py-3 text-[11px] font-bold uppercase tracking-wide transition ${
            active === id ? 'text-[var(--theme-accent)]' : 'text-zinc-500 hover:text-zinc-300'
          }`}
        >
